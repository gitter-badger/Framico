...
- Framico переписан на препроцессор SASS
 - В файле `config` можно изменять глобальные цвета
 - В файле `config` можно изменять количество блоков для сетки
- Возможно теперь загружать через `bower` и `npm`
- Полностью удален JS (возможно в будущем будет появляться)
 - Удалена навигация
 - Удален слайдер
 - Удален модальные окна
 - Удалены вкладки
- Удален Hint.css
- Сетка (Grid) переделана под flexbox
 - Добавлены адаптивные классы
 - Добавлены `offset`
- У фонов доступен новый класс
- Большинство `px` были переписаны на `rem`
- Кнопки переделаны под стиль Material `CSS`
- Переделано большинство имен классов
- Добавлен dropdown на CSS
- Добавлен `footer`
- Формы переделаны под стиль Material `CSS`
 - Добавлены стили для `checkbox` и `radio`
- Для таблиц был добавлен класс адаптивности
- Много мелких исправлений

## 1.7.6 (15.02.2016)

- Глобальное обновление кнопок
 - Кнопка `default` переименована в `white`
 - Вместо кнопки `default` теперь прозрачная кнопка
 - Измнен `border` во всех кнопках
 - Кнопка `white` переделана
 - Добавлена тень к тексту в кнопках
 - К кнопке `white` больше нельзя добавлять фон
 - Добавлены тени при активной кнопке
 - Оптимизирован код
